...
const navigation = computed(() => {
    ...
    // Add Kick tab if available
    if (stats.value?.application?.kick?.available) {
        items.push({ name: 'Kick', path: '/kick', icon: 'kick' });
    }
        ...
        <aside class="flex w-64 flex-col border-r border-zinc-800 bg-zinc-900">
            ...
            <nav class="flex-1 space-y-1 p-3">
                ...
                >
                    ...
                    <svg v-else-if="item.icon === 'kick'" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
